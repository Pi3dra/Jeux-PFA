...
let handle_input () =
  let () =
    ...
    | KeyUp s ->
        ...
        if s = "z" then
          Player.stop_players_y()
        (*
        TODO ne pas arreter le player si il est dans l'air*)
        else if s <> " " && not (Hashtbl.mem p#playerstate#get OnAirDown) && not (Hashtbl.mem p#playerstate#get OnAirUp) then
          Player.stop_players()
    | Quit -> exit 0
    | _ -> ()
  in
